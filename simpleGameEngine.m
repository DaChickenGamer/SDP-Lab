...
classdef simpleGameEngine < handle
    ...
    methods
        ...
        % TODO: Make standard since it's the game engine and shouldn't be
        % game specific
        function showCreaturePopup(obj, creatureName, texture)
            % Create a new figure for the popup
            popupFig = figure('Name', 'A Wild Creature Appears!', ...
                              'NumberTitle', 'off', ...
                              'MenuBar', 'none', ...
                              'ToolBar', 'none', ...
                              'Color', [1 1 1], ...
                              'Position', [600, 400, 300, 300]);
            
            % Display the sprite image (assumed to be a 3D RGB array)
            axes('Parent', popupFig, 'Position', [0.1 0.3 0.8 0.6]);
            imshow(obj.sprites{texture});
            ...
            % Create the "Capture" button at the bottom
            uicontrol('Style', 'pushbutton', ...
                      'String', 'Capture', ...
                      'FontSize', 12, ...
                      'Units', 'normalized', ...
                      'Position', [0.35 0.02 0.3 0.1], ...
                      'Callback', @(src, event) close(popupFig));  % Close popup on click
        end
    end
end